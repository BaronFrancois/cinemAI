...
function systemInstruction(tab, manifestSummary) {
  ...
  return [
    ...
    "Pour une revue narrative, propose narrativeStates (assetId, property, before, after) sur les références liées aux plans et narrativeTransition (unspecified, direct, ellipsis). Décris seulement des faits explicites ou une proposition clairement annoncée ; laisse les valeurs inconnues vides. Si un objet est repris dans un plan, son état before doit rester celui de la fin du précédent, et son état after reflète la reprise. Ne confonds pas coupe de caméra et ellipse, ni libellés différents et preuve visuelle. Toute correction doit préserver les plans voisins et être validée.",
